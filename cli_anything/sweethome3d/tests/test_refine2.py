...
class TestFurnitureGroupsCore:
    ...
    def test_create_moves_pieces_into_group(self):
        h = self._seed_home()
        grp = group_core.create_group(h, "DiningSet", piece_idents=["Chair", "Table"])
    ...
    def test_set_group_properties(self):
        h = self._seed_home()
        grp = group_core.create_group(h, "Set", piece_idents=["Chair", "Table"])
        group_core.set_group_properties(h, grp.id, name="Renamed", visible=False, price="500.00")
    ...
    def test_round_trip_group_xml(self, tmp_path):
        h = self._seed_home()
        group_core.create_group(h, "DiningSet", piece_idents=["Chair", "Table"])
...
class TestFurnitureGroupsCLI:
    def test_create_list_ungroup(self, tmp_path):
        ...
        r = _run(
            ["--project", sh3d, "--json", "group", "create", "Dining", "--pieces", "Chair,Table"]
        )
    ...
    def test_set_renames(self, tmp_path):
        ...
        _run(["--project", sh3d, "group", "set", "Set", "--name", "Renamed", "--hidden"])
        sess = Session.open(sh3d)
    ...
    def test_delete_removes_members(self, tmp_path):
        ...
        _run(["--project", sh3d, "group", "create", "Set", "--pieces", "Chair,Table"])
...
class TestMaterialsCore:
    def _seed(self) -> Home:
        ...
        furn_core.add_piece(h, "Sofa", 100, 100, width=200, depth=90, height=80)
        return h

    def test_set_creates_material(self):
        h = self._seed()
        mat = mat_core.set_material(h, "Sofa", "Cushion", color=0xFF8080FF, shininess=0.4)
    ...
    def test_set_with_texture(self):
        h = self._seed()
        mat = mat_core.set_material(h, "Sofa", "Cushion", texture_id="eTeks#marbleWall")
    ...
    def test_round_trip_xml(self, tmp_path):
        h = self._seed()
        mat_core.set_material(
            h, "Sofa", "Cushion", color=0xFFAABBCC, shininess=0.5, texture_id="eTeks#marbleWall"
        )
...
class TestSashesCore:
    def _door_home(self) -> Home:
        ...
        furn_core.add_door(h, "Door", 100, 100, width=80, depth=10, height=200)
        return h

    def test_add_sash(self):
        h = self._door_home()
        sash = sash_core.add_sash(
            h, "Door", xAxis=0, yAxis=0, width=1.0, startAngle=0, endAngle=1.5
        )
        assert sash.xAxis == 0
    ...
    def test_add_sash_rejects_non_door(self):
        h = proj_core.new_home("S")
        furn_core.add_piece(h, "Sofa", 0, 0, width=200, depth=90, height=80)
        with pytest.raises(ValueError):
            sash_core.add_sash(h, "Sofa", xAxis=0, yAxis=0, width=1, startAngle=0, endAngle=1)

    def test_add_sash_rejects_bad_fraction(self):
        h = self._door_home()
        with pytest.raises(ValueError):
            sash_core.add_sash(h, "Door", xAxis=5, yAxis=0, width=1, startAngle=0, endAngle=1)

    def test_delete_sash(self):
        h = self._door_home()
        sash_core.add_sash(h, "Door", xAxis=0, yAxis=0, width=1.0, startAngle=0, endAngle=1.5)
        sash_core.add_sash(h, "Door", xAxis=0.5, yAxis=0, width=0.5, startAngle=0, endAngle=1.0)
    ...
    def test_round_trip(self, tmp_path):
        h = self._door_home()
        sash_core.add_sash(
            h, "Door", xAxis=0.0, yAxis=0.0, width=0.95, startAngle=0, endAngle=1.5707
        )
...
class TestLightEmittersCore:
    def _light_home(self) -> Home:
        ...
        furn_core.add_light(h, "Lamp", 100, 100, width=20, depth=20, height=20)
        return h

    def test_add_source(self):
        h = self._light_home()
        src = light_core.add_source(h, "Lamp", x=0, y=0, z=0.5, color=0xFFFFFF80, diameter=2.0)
    ...
    def test_add_source_rejects_non_light(self):
        h = proj_core.new_home("L")
        furn_core.add_piece(h, "Chair", 0, 0, width=60, depth=60, height=90)
        with pytest.raises(ValueError):
            light_core.add_source(h, "Chair", x=0, y=0, z=0, color=0xFFFFFFFF)
    ...
    def test_round_trip(self, tmp_path):
        h = self._light_home()
        light_core.add_source(h, "Lamp", x=0, y=0, z=0.5, color=0xFFFFFF80, diameter=2.5)
        light_core.add_material(h, "Lamp", "Shade")
...
class TestLightEmittersCLI:
    def test_source_add_list_delete(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(["--project", sh3d, "furniture", "add-light", "Lamp", "100", "100"])
        _run(
            [
                "--project",
                sh3d,
...
class TestShelvesCore:
    def _shelf_home(self) -> Home:
        ...
        piece = furn_core.add_piece(
            h, "Bookcase", 0, 0, width=80, depth=30, height=180, kind="pieceOfFurniture"
        )
    ...
    def test_add_box_shelf(self):
        ...
        sh = shelf_core.add_box_shelf(
            h, "Bookcase", xLower=-30, yLower=-10, zLower=0, xUpper=30, yUpper=10, zUpper=20
        )
        assert sh.zUpper == 20
    ...
    def test_add_box_shelf_rejects_bad_bounds(self):
        h = self._shelf_home()
        with pytest.raises(ValueError):
            shelf_core.add_box_shelf(
                h, "Bookcase", xLower=0, yLower=0, zLower=0, xUpper=0, yUpper=10, zUpper=10
            )
    ...
    def test_round_trip(self, tmp_path):
        ...
        shelf_core.add_box_shelf(
            h, "Bookcase", xLower=-30, yLower=-10, zLower=60, xUpper=30, yUpper=10, zUpper=120
        )
...
class TestPrintSettingsCore:
    ...
    def test_round_trip(self, tmp_path):
        ...
        h = proj_core.new_home("P")
        lvl_core.add_level(h, "Ground")
        print_core.set_print(
            h,
            paperWidth=297,
            paperHeight=420,
            paperOrientation="LANDSCAPE",
            headerFormat="Page $page",
            planScale=50,
...
class TestPrintSettingsCLI:
    ...
    def test_level_filter(self, tmp_path):
        ...
        _run(["--project", sh3d, "print", "set", "--paper-width", "210", "--paper-height", "297"])
...
class TestStoredCameraKindFix:
    ...
    def test_observer_view_keeps_observer_kind(self, tmp_path):
        ...
        obs_stored = [
            c for c in root.findall("observerCamera") if c.get("attribute") == "storedCamera"
        ]
    ...
    def test_top_view_keeps_top_kind(self, tmp_path):
        ...
        with zipfile.ZipFile(p) as z:
            xml = z.read("Home.xml").decode()
        root = ET.fromstring(xml)
        cam_stored = [c for c in root.findall("camera") if c.get("attribute") == "storedCamera"]
    ...
    def test_cli_save_preserves_kind(self, tmp_path):
        ...
        _run(["--project", sh3d, "camera", "save", "overview", "--kind", "topCamera"])
        sess = Session.open(sh3d)
        top_view = next(c for c in sess.home.storedCameras if c.name == "overview")
...
"""
    ...
    def test_from_project_dedupes(self):
        h = proj_core.new_home("P")
        furn_core.add_piece(h, "Sofa", 0, 0, width=200, depth=90, height=80, catalogId="eTeks#sofa")
        furn_core.add_piece(
            h, "Sofa2", 300, 0, width=200, depth=90, height=80, catalogId="eTeks#sofa"
        )  # duplicate id
        furn_core.add_piece(
            h, "Chair", 0, 200, width=45, depth=50, height=90, catalogId="eTeks#chair"
        )
    ...
    def test_from_project_walks_groups(self):
        ...
        furn_core.add_piece(h, "Sofa", 0, 0, width=200, depth=90, height=80, catalogId="eTeks#sofa")
        furn_core.add_piece(
            h, "Chair", 300, 0, width=45, depth=50, height=90, catalogId="eTeks#chair"
        )
    ...
    def test_cli_from_project_kind_filter(self, tmp_path):
        ...
        _run(["--project", sh3d, "furniture", "add-door", "Door", "100", "0"])
        r = _run(["--project", sh3d, "--json", "catalog", "from-project", "--kind", "doorOrWindow"])
        entries = json.loads(r.stdout)
...
class TestValidateCore:
    ...
    def test_unknown_catalog_warning(self):
        ...
        with warnings.catch_warnings():
            ...
            furn_core.add_piece(
                h, "Mystery", 0, 0, width=50, depth=50, height=50, catalogId="madeup#widget"
            )
        report = validate_core.validate(h)
        assert any(f.code == "furniture.unknown_catalog" for f in report.warnings)
    ...
    def test_real_example_validates_clean(self):
        ...
        ex_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "..", "examples", "Home-Clean-Base-RAL.sh3d"
        )
...
class TestRoomAreaAndInfo:
    def test_area_cli_m2(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(["--project", sh3d, "room", "rectangle", "0", "0", "500", "400", "--name", "R"])
...
class TestWallLengthAndInfo:
    def test_length_units(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        r = _run(
            ["--project", sh3d, "--json", "wall", "add", "0", "0", "300", "400"]
    ...
    def test_info_angle(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        r = _run(["--project", sh3d, "--json", "wall", "add", "0", "0", "100", "100"])
        wid = json.loads(r.stdout)["id"]
...
class TestFurnitureInfo:
    def test_info_includes_materials(self, tmp_path):
        ...
        _run(["--project", sh3d, "material", "set", "Sofa", "Cushion", "--color", "#AABBCC"])
...
class TestFindRoomsExtensions:
    def test_unnamed_filter(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(["--project", sh3d, "room", "rectangle", "0", "0", "500", "400", "--name", "Named"])
    ...
    def test_area_range_filter(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(
            ["--project", sh3d, "room", "rectangle", "0", "0", "100", "100", "--name", "Tiny"]
        )  # 1 m²
        _run(
            ["--project", sh3d, "room", "rectangle", "0", "200", "500", "400", "--name", "Big"]
        )  # 20 m²
        r = _run(["--project", sh3d, "--json", "find", "rooms", "--area-max", "5"])
        rooms = json.loads(r.stdout)
        assert [r["name"] for r in rooms] == ["Tiny"]
        r = _run(["--project", sh3d, "--json", "find", "rooms", "--area-min", "10"])
        rooms = json.loads(r.stdout)
...
class TestCameraTime:
    ...
    def test_rejects_invalid_date(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        r = _run(
            ["--project", sh3d, "camera", "time", "--month", "13", "--day", "1", "--hour", "12"],
            check=False,
        )
        assert r.returncode != 0
...
class TestLevelDuplicateCore:
    ...
    def test_elevation_defaults_to_stacking(self):
        h = proj_core.new_home("L")
        lvl_core.add_level(h, "Ground")  # elevation=0, height=250, floor=12
        new_lvl = lvl_core.duplicate_level(h, "Ground", new_name="Upper")
    ...
    def test_offset_translates_geometry(self):
        ...
        new_lvl = lvl_core.duplicate_level(
            h, "Ground", new_name="Annexe", offset_x=1000, offset_y=500
        )
    ...
    def test_skip_furniture_flag(self):
        h = proj_core.new_home("L")
        src = lvl_core.add_level(h, "Ground")
        furn_core.add_piece(h, "Sofa", 100, 100, width=200, depth=90, height=80, level=src.id)
        new_lvl = lvl_core.duplicate_level(h, "Ground", new_name="Upper", include_furniture=False)
...
class TestLevelDuplicateCLI:
    def test_duplicate_cli(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(["--project", sh3d, "level", "add", "Ground"])
        _run(["--project", sh3d, "wall", "rectangle", "0", "0", "500", "400", "--level", "Ground"])
        r = _run(["--project", sh3d, "--json", "level", "duplicate", "Ground", "--name", "Upper"])
        new_lvl = json.loads(r.stdout)
        assert new_lvl["name"] == "Upper"

        # Find walls on Upper
        r = _run(["--project", sh3d, "--json", "find", "walls", "--level", "Upper"])
    ...
    def test_duplicate_with_explicit_elevation(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        _run(["--project", sh3d, "level", "add", "Ground"])
        _run(["--project", sh3d, "wall", "add", "0", "0", "100", "0", "--level", "Ground"])
        r = _run(
            [
                "--project",
                sh3d,
...
class TestWallSplitCore:
    ...
    def test_split_preserves_properties(self):
        ...
        w = walls_core.add_wall(h, 0, 0, 500, 0, thickness=14, height=300, leftSideColor=0xFFAABBCC)
        w.leftSideBaseboard = Baseboard(thickness=1.5, height=12, color=0xFFFFFFFF)
        h1, h2 = walls_core.split_wall(h, w.id, at_x=250, at_y=0)
        for half in (h1, h2):
...
class TestWallSplitJoinCLI:
    ...
    def test_join_cli(self, tmp_path):
        ...
        _run(["project", "new", "-o", sh3d])
        r1 = _run(["--project", sh3d, "--json", "wall", "add", "0", "0", "250", "0"])
        r2 = _run(["--project", sh3d, "--json", "wall", "add", "250", "0", "500", "0"])
...
class TestRefine2Workflow:
    ...
    def test_full_decorated_room(self, tmp_path):
        ...
        _run(["--project", sh3d, "group", "create", "SeatingSet", "--pieces", "Sofa,Chair"])
        _run(
            [
                "--project",
                sh3d,
        ...
        # Door with sash
        _run(["--project", sh3d, "furniture", "add-door", "EntryDoor", "250", "5", "--width", "80"])
        _run(
            [
                "--project",
                sh3d,
        ...
        sofa_in_group = next(
            p
            for p in h.furnitureGroups[0].furniture
            if isinstance(p, PieceOfFurniture) and p.name == "Sofa"
        )
