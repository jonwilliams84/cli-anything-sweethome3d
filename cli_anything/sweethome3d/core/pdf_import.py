...
_SHELL_META = re.compile(
    r"[;&|`$<>!\"'\s]"
    # reject any byte that is not a printable ASCII character or safe path char
    r"|[\x00-\x1f\x7f]"
)


def _validate_shell_safe(value: str, label: str) -> None:
    """Raise ValueError if value contains shell metacharacters or control bytes.

    We intentionally do NOT invoke the command through a shell (``shell=False`` in
    subprocess.run), but the user-supplied command is ultimately passed to the OS
    via argv of the spawned process — defence in depth by blocking any character
    that could cause unexpected shell expansion if the command is ever run via
    a shell wrapper or piped through sh(1).  This also prevents argument-injection
...
def run_model(png_path, out_json, *, model_cmd=None):
    ...
    if isinstance(cmd, str):
        cmd = shlex.split(cmd)
    for token in cmd:
        _validate_shell_safe(token, "command token")

    # B603: validate the paths that will be substituted into the template.
    _validate_shell_safe(png_path, "png_path")
    _validate_shell_safe(out_json, "out_json")

    if not os.path.isfile(png_path):
        raise FileNotFoundError(f"input PNG does not exist: {png_path}")

    cmd = [c.replace("{in}", png_path).replace("{out}", out_json) for c in cmd]

    # B603: after substitution, re-validate every argument (substitution could
    # have inserted a metacharacter into an unsuspicious-looking placeholder).
    for token in cmd:
        _validate_shell_safe(token, "substituted command token")

    subprocess.run(cmd, check=True)  # nosec subprocess — tokens are validated above
