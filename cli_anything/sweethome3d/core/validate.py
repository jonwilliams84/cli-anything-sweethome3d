        ...
        if a < 1.0:  # less than 1 cm²
            ...
            out.append(
                Finding(
                    code="room.tiny",
                    severity="warning",
                    message=f"room area is only {a / 10000:.2f} m² (likely an importer fragment)",
                    target_id=r.id,
                    target_name=r.name,
                    extra={"area_cm2": a},
                )
        ...
        if f.width <= 0 or f.depth <= 0 or f.height <= 0:
            out.append(
                Finding(
                    ...
                    severity="error",
                    message=(
                        f"piece has non-positive dimension: w={f.width} d={f.depth} h={f.height}"
                    ),
                    target_id=f.id,
                    target_name=f.name,
                )
            ...
            if level_ref and level_ref not in valid_ids:
                out.append(
                    Finding(
                        ...
                        severity="error",
                        message=(f"{source_label} references missing level {level_ref!r}"),
...
def _check_doors_bound(home: Home) -> list[Finding]:
    ...
    for f in _iter_all_pieces(home):
        ...
        for w in home.walls:
            if w.level != f.level:
                continue
            d = point_to_segment_dist(f.x, f.y, w.xStart, w.yStart, w.xEnd, w.yEnd)
            if d <= radius:
                hit = True
                break
        if not hit:
...
def validate(home: Home, *, include_info: bool = True) -> ValidationReport:
