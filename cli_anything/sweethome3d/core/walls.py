    ...
    *,
    ...
    for other in home.walls:
        ...
        for slot in ("wallAtStart", "wallAtEnd"):
            ...
            ox, oy = (
                (other.xStart, other.yStart) if slot == "wallAtStart" else (other.xEnd, other.yEnd)
            )
            d_to_h1_start = math.hypot(ox - half1.xStart, oy - half1.yStart)
            d_to_h2_end = math.hypot(ox - half2.xEnd, oy - half2.yEnd)
            setattr(other, slot, half1.id if d_to_h1_start <= d_to_h2_end else half2.id)
...
def join_walls(
    home: Home, first: str, second: str, *, endpoint_tol_cm: float = 1.0, angle_tol_deg: float = 2.0
) -> Wall:
    ...
    if a is None:
        raise KeyError(f"wall not found: {first}")
    if b is None:
        raise KeyError(f"wall not found: {second}")
    if a is b:
        raise ValueError("cannot join a wall to itself")
    if a.level != b.level:
        raise ValueError(f"walls are on different levels ({a.level!r} vs {b.level!r})")
    if abs(a.thickness - b.thickness) > 0.1:
        raise ValueError(f"walls have different thickness ({a.thickness} vs {b.thickness})")
    if abs(a.height - b.height) > 0.1:
        raise ValueError(f"walls have different height ({a.height} vs {b.height})")
    ...
    if _close(a_e, b_s):
        ...
        raise ValueError(f"walls don't share an endpoint within {endpoint_tol_cm} cm")
...
def connect_walls(
    home: Home, first: str, second: str, *, at: str = "end-start"
) -> tuple[Wall, Wall]:
