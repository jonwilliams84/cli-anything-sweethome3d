...
def _furniture_polygon(f) -> list[tuple[float, float]]:
    ...
    return [(f.x + cx * cos_a - cy * sin_a, f.y + cx * sin_a + cy * cos_a) for cx, cy in corners]
...
def _resolve_level(home: Home, spec: str) -> str:
    ...
    raise ValueError(f"unknown level spec: {spec!r}. Known ids/names: {known!r}")


def to_svg(home: Home, *, padding: float = 50, scale: float = 1.0, level: str | None = None) -> str:
