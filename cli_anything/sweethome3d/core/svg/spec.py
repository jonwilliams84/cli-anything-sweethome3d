...
DEFAULT_SPEC: dict = {
    ...
    "rooms": {
        ...
        "detection": {"min_area_cm2": 5000, "max_vertices": 200, "dedupe_centroid_cm": 10},
        "by_level": {
            "Ground": {"default_floor": "#9C7A4D", "overrides": {}},
            "1st Floor": {
                "default_floor": "#D8C6A4",
                "overrides": {
                    "Master": "#3A3A3D",
    ...
    * **anything else**: override replaces base wholesale (including
    ...
    for k, v in override.items():
        if k in out and isinstance(out[k], dict) and isinstance(v, dict):
            ...
            and out[k]
            and v
            and isinstance(out[k][0], dict)
            and isinstance(v[0], dict)
        ):
            merged_list = [
                deep_merge(out[k][i] if i < len(out[k]) else {}, v[i]) for i in range(len(v))
            ]
...
def _validate_enum(value, valid_set, key_path: str):
    ...
    if value is not None and value not in valid_set:
        raise ValueError(
            f"spec key '{key_path}': invalid value {value!r}. Valid values: {sorted(valid_set)}"
        )
...
def _validate_enums(cfg: dict):
    ...
    _validate_enum(
        walls.get("new_wall_pattern"), _VALID_WALL_PATTERNS | {None}, "walls.new_wall_pattern"
    )
    ...
    _validate_enum(
        photo.get("aspect_ratio"), _VALID_PHOTO_ASPECT_RATIOS, "environment.photo.aspect_ratio"
    )
    video = env.get("video") or {}
    _validate_enum(
        video.get("aspect_ratio"), _VALID_VIDEO_ASPECT_RATIOS, "environment.video.aspect_ratio"
    )

    prnt = cfg.get("print") or {}
    _validate_enum(
        prnt.get("paper_orientation"), _VALID_PRINT_ORIENTATIONS, "print.paper_orientation"
    )
...
def load_spec(spec) -> dict:
    ...
    def _validate(node, defaults, prefix=""):
        ...
        for k, v in node.items():
            ...
            if k not in defaults:
                warnings.warn(f"unknown spec key: {full}", stacklevel=2)
    ...
    if isinstance(spec, dict):
        ...
        _validate_enums(merged)
        return merged
    raise TypeError(f"spec must be None, dict, or path string — got {type(spec).__name__}")
