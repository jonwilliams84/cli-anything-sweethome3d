...
parser.add_argument(
    "--camera-json", dest="camera_json", default=None, help="Path to the sidecar .camera.json file"
)
parser.add_argument(
    "--view",
    dest="view",
...
try:
    ...
    print(f"BLENDER-RENDER: argument parsing failed (exit {e.code}). raw_args={raw_args}")
...
try:
    ...
    print(
        "BLENDER-RENDER: ERROR — bpy is not available. "
...
def _parse_color(hex_str: str, default=(1.0, 1.0, 1.0, 1.0)):
    ...
    try:
        ...
        print(
            f"BLENDER-RENDER: WARNING — could not parse colour '{hex_str}': {exc}. "
...
try:
    ...
    imported_ok = True
    print(
        f"BLENDER-RENDER: OBJ imported via bpy.ops.wm.obj_import "
...
if not imported_ok:
    try:
        ...
        imported_ok = True
        print(
            f"BLENDER-RENDER: OBJ imported via bpy.ops.import_scene.obj "
...
try:
    ...
    scene.view_settings.exposure = float(
        __import__("os").environ.get("SH3D_RENDER_EXPOSURE", "-0.35")
    )
...
if CAMERA_JSON and os.path.isfile(CAMERA_JSON):
    try:
        ...
        print(
            f"BLENDER-RENDER: WARNING — could not load camera JSON "
            f"'{CAMERA_JSON}': {exc}. Using defaults."
        )
        cam_info = None
else:
    if CAMERA_JSON:
        print(f"BLENDER-RENDER: WARNING — camera JSON '{CAMERA_JSON}' not found. Using defaults.")
    else:
...
def _fit_top_down_camera(bounds):
    ...
    rot = (0.0, 0.0, 0.0)
    print(
        f"BLENDER-RENDER: TOP-DOWN ortho cam centre=({cx:.2f},{cy:.2f}) "
...
def _fit_iso_camera(bounds):
    ...
    print(
        f"BLENDER-RENDER: ISO cam target=({cx:.2f},{cy:.2f},{cz:.2f}) "
...
if VIEW == "top" and bounds is not None:
    ...
    print(
        f"BLENDER-RENDER: SH3D camera x={x_sh3d} y={y_sh3d} z={z_sh3d} "
    ...
    try:
        ...
        print(
            f"BLENDER-RENDER: WARNING — centroid calculation failed: {exc}. Defaulting to origin."
        )
        cx, cy = 0.0, 0.0
...
if cam_info and "environment" in cam_info:
    ...
    env_wall_height = float(
        cam_info["environment"].get("wallHeight", float(cam_info.get("wallHeight", 250.0)))
    )
else:
...
try:
    ...
    print(
        f"BLENDER-RENDER: fill area light placed at {fill_obj.location} size={fill_data.size:.2f}"
    )
except Exception as exc:
...
try:
    ...
    if VIEW in ("top", "iso") and bounds is not None:
        ...
        if VIEW == "top":
            ...
            print(
                f"BLENDER-RENDER: camera placed at {cam_loc} "
            ...
            cam_obj.rotation_quaternion = direction.to_track_quat("-Z", "Y")
            print(
                f"BLENDER-RENDER: camera placed at {cam_loc} aiming at {tuple(target)} "
        ...
        and (
            float(cam_info["camera"].get("yaw", 0.0)) != 0.0
        ...
        print(
            f"BLENDER-RENDER: camera placed at {cam_loc} rot={cam_rot} "
        ...
        if centroid_verts:
            ...
            cam_obj.rotation_quaternion = direction.to_track_quat("-Z", "Y")
            print(
                f"BLENDER-RENDER: camera placed at {cam_loc} aiming at centroid {tuple(target)} "
...
print(f"view: {VIEW}")
if _use_ortho:
    print(f"camera: ortho_scale={_ortho_scale:.3f} loc={cam_loc} (source: {camera_source})")
else:
    print(
        f"camera: loc={cam_loc} rot={tuple(round(r, 4) for r in cam_rot)} "
